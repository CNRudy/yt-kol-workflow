...
## 6. 飞书表 ID 映射（**不在公开仓库中给出真实 ID**）

> 本仓库不绑定任何飞书 Base；以下仅为**字段命名规范示例**。
> 真实 table_id 列表存放于项目内 `docs/TABLE_MAP.md`（不入公开仓库，含 Base/表结构/字段规范），
> 运行人员应通过 `docs/TABLE_MAP.md` 或飞书后台 URL 自取。

每个 Base 至少需要 4 张表（按用途）：

| 表（命名示例） | 用途 | 关键字段 | 通过环境变量 |
|---|---|---|---|
| 详情表 | 频道/红人全量数据 | Channel ID / Channel Name / 订阅数 / 来源关键词 / 领域 / 评分列 | `FEISHU_TABLE_DETAIL` |
| 视频数据表 | 视频筛选+评分输入 | Video ID / 标题 / Tags / Views / 互动率 / 描述 | `FEISHU_TABLE_VIDEO` |
| 网红视频表 | 每频道 N 条最近视频 | Channel ID / 标题 / Views / 互动率 | `FEISHU_TABLE_INFLUENCER_VIDEO` |
| 搜索任务表 | 每次搜索 1 行 | 关键词 / 最大结果数 / 时间 | `FEISHU_TABLE_TASK` |
| 红人表（派生，可选） | 增量同步的精简表 | Channel ID / 推荐理由 / 开发状态 / 备注 | `FEISHU_HONGREN_TABLE_ID` |

> 字段规范（2026-09-07 起）：保留 `来源关键词`、新增 `领域` 单选（12 选项），删除 `匹配关键词`/`匹配排序组`/`分类`/`KOL Name`；报价/佣金/合作进度在**金山文档**而非飞书。
