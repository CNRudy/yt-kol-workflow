...
`search` 和 `batch` 共用同一组筛选参数，决定"哪些视频算合格内容"。筛选发生在 **视频层面**：一条视频达标，它所属的频道才会被识别为候选网红。下面以 `--min-views`、`--min-engagement`、`--filter-mode` 等为例详细说明。

### 1. 核心阈值（作用在单条视频上）

| 参数 | 默认值 | 达标规则 | 通俗解释 |
|------|--------|----------|----------|
| `--min-views N` | 10000 | `播放量 > N`（**严格大于**） | 视频播放量下限。例如 `--min-views 5000` 表示只保留播放量 ≥ 5001 的视频 |
| `--min-engagement N` | 3.0 | `互动率 > N%` 且 `播放量 > 0` | 视频互动率下限（单位 %）。互动率 = (点赞+评论+收藏)/播放量。播放量为 0 的视频永远不达标 |
| `--min-subscribers N` | 0 | `频道订阅数 ≥ N` | 频道级体量门槛（看的是**整个频道**，不是单条视频）。默认 0 = 不限制 |

### 2. 两个阈值如何组合：`--filter-mode`

`--min-views` 和 `--min-engagement` 是两条独立的"及格线"，`--filter-mode` 决定它们怎么组合：

- **`or`（默认）**：两条线**任一达标**即可。最宽松，适合广撒网找潜在网红。
- **`and`**：两条线**必须同时达标**。只保留"既有流量又有粘性"的视频。
- **`weighted`（加权均衡）**：不要求单条线达标，两项各占 50% 权重，按相对阈值的完成度打分：
  `加权分 = 0.5 × min(播放量/阈值, 1) + 0.5 × min(互动率/阈值, 1)`
  加权分 **≥ 1.0** 才合格。允许"播放量略低但互动率很高"（或反之）的视频互补通过。

> 例：阈值 `--min-views 10000 --min-engagement 3`，一条视频播放量 8000、互动率 4%：
> - `or`：互动率 4% > 3% → 达标 ✅
> - `and`：播放量 8000 < 10000 → 不达标 ❌
> - `weighted`：0.5×(8000/10000) + 0.5×(4/3) = 0.5×0.8 + 0.5×1.0 = 0.9 < 1.0 → 不达标 ❌

### 3. 搜索侧过滤（交给 YouTube 服务器预筛）

这些参数在拉取搜索结果时就生效，能减少后续处理的视频量：

| 参数 | 可选值 | 作用 |
|------|--------|------|
| `--video-duration` | any / short / medium / long | 只搜指定长度的视频 |
| `--video-caption` | any / closedCaption / none | 按字幕状态过滤 |
| `--video-definition` | any / high / standard | 按清晰度过滤 |
| `--published-after` | YYYY-MM-DD | 视频发布时间不早于该日期 |
| `--published-before` | YYYY-MM-DD | 视频发布时间不晚于该日期 |
| `--region` | 地区代码（默认 US） | 搜索地区 |
| `--lang` | 语言代码（默认 en） | 搜索语言 |

### 4. 其他常用参数

- `--max-results N`：每个关键词**最多拉取**多少条搜索结果（默认 100）。这是"搜索入口数"，不是最终合格数；最终数量还受上面阈值影响。
- `--sort-order`：搜索结果排序方式，可选 `relevance`(相关度) / `viewCount`(播放量) / `date`(时间) / `rating`(评分) / `title`(标题)。不指定会在交互中询问。
- `--no-detail`：只执行"搜索 + 筛选"，**不抓取**网红详情（频道描述、联系方式等）。适合先快速验证筛选效果。
...
> 各筛选参数（`--min-views` / `--min-engagement` / `--filter-mode` 等）的含义与示例见上方 **[筛选参数详解](#筛选参数详解)** 小节。
...
选项:
  --sort-order {relevance,viewCount,date,rating,title}
                                            排序策略（不指定则交互询问）
  --max-results N                          每个关键词最多拉取的搜索结果数（默认100）
  --no-detail                              仅搜索+筛选，不抓取网红详情（频道描述/联系方式等）
  --feishu-app-token TOKEN                 可选：指定飞书 Base URL 或 app_token
  --no-feishu                              跳过飞书写入
  --no-excel                               跳过本地Excel输出
  --min-views N                            视频播放量下限，规则为 播放量 > N（默认10000，详见筛选参数详解）
  --min-engagement N                       视频互动率下限(百分比)，规则为 互动率 > N% 且播放量>0（默认3.0）
  --filter-mode {or,and,weighted}          视频筛选逻辑：or=任一达标 / and=双达标 / weighted=加权均衡（默认 or）
  --min-subscribers N                      频道最低订阅数(频道级，默认0=不限制)
...
# 筛选参数含义见上方「筛选参数详解」。常用写法示例:
#   --min-views 5000              只保留播放量 > 5000 的视频
#   --min-engagement 2            互动率阈值降到 2%
#   --filter-mode and            播放量与互动率必须同时达标
#   --min-subscribers 10000      频道订阅数至少 1 万
#   --video-duration medium      只搜中等长度视频
#   --published-after 2025-01-01 只要该日期之后发布的视频
# 其他开关:
#   --no-feishu / --no-excel     跳过对应输出
#   --playlist / --search        指定获取网红视频的方式
#   --yes                        跳过确认直接运行
```
