...
def _estimate_remaining_cost(
    ...
    # Matches run_phase_d's recent_video_count default. Fetching 25 instead of
    # 10 videos costs the same playlistItems page and is what makes the email /
    # Amazon-promo detection actually hit.
    recent_video_batches = (channel_count * 25 + 49) // 50
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp_sync.add_argument(
        "--skip-hongren",
        action="store_true",
        help="跳过派生「红人表」的自动同步（默认同步完主表后会一并增量更新红人表）",
    )
    ...
    sp_refresh.add_argument(
        "--fields",
        choices=["all", "name", "activity", "assessment", "rep-title", "promo"],
        default="all",
        help="只刷新指定字段组（默认 all）。promo=亚马逊推广经验+从视频描述补邮箱",
    )
...
def handle_sync_workbook(args):
    ...
    # 派生「红人表」自动跟随更新：主表同步完成后一并增量同步。
    # 背景：过去红人表靠 build_hongren_table.py「删表重建」生成，无法自动更新，
    # 且每次重建都会换 table_id 导致链接失效。改为同步完主表后增量 upsert。
    if (not args.dry_run and not getattr(args, "skip_hongren", False)
            and args.table in ("all", "influencers")):
        try:
            print("\n[派生表] 增量同步「红人表」…")
            import os as _os
            import sys as _sys
            _sys.path.insert(0, _os.path.dirname(_os.path.abspath(__file__)))
            import sync_hongren_table
            sync_hongren_table.sync()
        except Exception as exc:  # 派生表失败不应影响主表同步结果
            print(f"  ! 红人表同步失败（不影响主表）: {exc}")
