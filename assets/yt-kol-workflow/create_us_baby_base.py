#!/usr/bin/env python3
"""为 WEMOH 美国婴儿车载摄像头红人（US + 未知地区），新建一个独立飞书多维表格(Base)并写入。

复用 create_cfc1_base.py 的字段推断 + 批量写入逻辑，仅改源文件 / Base 名 / 表名，
并对单选列脏值(如 "None")做清洗，避免飞书单选选项被污染。
"""
...
SRC_XLSX = "output/us_baby_sync/influencers_rescored.xlsx"
BASE_NAME = "WEMOH美国婴儿车载摄像头红人库"
TABLE_NAME = "US婴儿摄像头红人(含未知地区)"
...
def batch_write(base_token, table_id, cols, records, batch=200, pause=0.3):
    ok = fail = 0
    tmp = os.path.join(os.getcwd(), "._us_baby_tmp.json")
    for i in range(0, len(records), batch):
        ...
        j = run(["base", "+record-batch-create", "--as", "user",
                 ...
                 "--json", "@._us_baby_tmp.json", "--format", "json"], expect_ok=False)
...
def main():
    ...
    ap.add_argument("--base-token", default=None)
    ...
    for s in specs:
        ...
        print(f"    {s['name']:20} -> {s['type']}{extra}")
